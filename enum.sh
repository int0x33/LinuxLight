...
echo "Check 6: What applications are installed? What version are they? Are they currently running?"
...
echo "Check 7: Any of the service(s) settings misconfigured? Are any (vulnerable) plugins attached?"
...
echo "Check 9: Any plain text usernames and/or passwords?"
...
echo "Check 10: What NIC(s) does the system have? Is it connected to another network?"
/sbin/ifconfig -a 2>/dev/null
cat /etc/network/interfaces 2>/dev/null
cat /etc/sysconfig/network 2>/dev/null

echo "Check 11: What are the network configuration settings? What can you find out about this network? DHCP server? DNS server? Gateway?"
...
echo "Check 12: What other users & hosts are communicating with the system?"
lsof -i 2>/dev/null
...
echo "Check 13: Whats cached? IP and/or MAC addresses"
arp -e 2>/dev/null
route 2>/dev/null
/sbin/route -nee 2>/dev/null


echo "Check 14: Who are you? Who is logged in? Who has been logged in? Who else is there? Who can do what?"
id 2>/dev/null
who 2>/dev/null
w 2>/dev/null
last 2>/dev/null
...
echo "Check 15: What sensitive files can be found?"
cat /etc/passwd 2>/dev/null
cat /etc/group 2>/dev/null
cat /etc/shadow 2>/dev/null
ls -alh /var/mail/ 2>/dev/null

echo "Check 16: Anything interesting in the home directorie(s)? If it's possible to access"
ls -ahlR /root/ 2>/dev/null
ls -ahlR /home/ 2>/dev/null

echo "Check 17: Are there any passwords in; scripts, databases, configuration files or log files? Default paths and locations for passwords"
cat /var/apache2/config.inc 2>/dev/null
cat /var/lib/mysql/mysql/user.MYD 2>/dev/null
cat /root/anaconda-ks.cfg 2>/dev/null

echo "Check 18: What has the user being doing? Is there any password in plain text? What have they been edting?"
...
echo "Check 19: What user information can be found?"
cat ~/.bashrc 2>/dev/null
cat ~/.profile 2>/dev/null
cat /var/mail/root 2>/dev/null
cat /var/spool/mail/root 2>/dev/null

echo "Check 20: Can private-key information be found?"
...
echo "Check 22: What can be found in /var/ ?"
...
echo "Check 23: Any settings/files (hidden) on website? Any settings file with database information?"
...
echo "Check 24: Is there anything in the log file(s) (Could help with 'Local File Includes'!)"
...
echo "Check 25: How are file-systems mounted?"
mount 2>/dev/null
df -h 2>/dev/null

echo "Check 26: Are there any unmounted file-systems?"
cat /etc/fstab 2>/dev/null

echo "Check 27: What 'Advanced Linux File Permissions' are used? Sticky bits, SUID & GUID"
...
echo "Check 28: Where can written to and executed from? A few 'common' places: /tmp, /var/tmp, /dev/shm"
...
echo "Check 30: What development tools/languages are installed/supported?"
find / -name perl* 2>/dev/null
find / -name python* 2>/dev/null
find / -name gcc* 2>/dev/null
find / -name cc 2>/dev/null

echo "Check 40: How can files be uploaded?"
